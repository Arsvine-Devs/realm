...
本次架构快照的原始调查截止时间为 **2026-09-13**；接口接通复核更新于 **2026-09-15**。其中流水线和整改文档包含迁移前历史记录，当前运行时以 Realm/Platform 源码、环境契约和已发布服务端点为准。
...
| ---------------------------- | --------------------------------------- | ----------- | --------------------------- | -------------------------------------------- |
| Realm                        | `Arsvine-Devs/realm` / `master`         | `4b8f799`   | `CURRENT`，干净             | 主站源码、Realm 内部数据、资产构建工具       |
| Admin                        | `Arsvine-Devs/platform` / `main`        | `a0cc4a0`   | `CURRENT`，干净             | 内容控制面、账户和同步任务                   |
...
独立 Content 仓库曾只读浅克隆到本机的 `C:\dev\arsvine-content`，现已标记为废弃输入；该本地路径和浅克隆方式不属于产品契约，也不纳入本轮治理。
