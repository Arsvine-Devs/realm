...
> 文档状态：规划草案，部分模型来自已退出的旧 Admin/GitHub/X/cron 形态。当前服务边界以 Realm 与 Platform 的源码和当前文档为准；本文不构成现行迁移或运维步骤。

本文是未来迁移的边界和顺序说明，不是本次执行授权。当前不迁移数据库、内容、COS、DNS、域名、部署或供应商账户。临时 `anti-fraud-quiz` 是明天活动站，活动后按独立清理流程删除，不纳入 VPS 迁移目标。
...
flowchart TB
  ...
  RP --> ADMIN[Admin Next process\nnext start]
  RP --> DOCS[Static docs server\ndoc_build]

  REALM --> PG[PostgreSQL\nvisitor schema]
  ADMIN --> PG2[PostgreSQL\nadmin schema]
  REALM --> REDIS[Redis/Valkey\nrate-limit state]
  ADMIN --> REDIS
  REALM --> CONTENT[ContentRepository\ninitially GitHub]
  ADMIN --> CONTENT
  REALM --> BLOBS[BlobStore/CDN\ninitially COS + EdgeOne]
  ADMIN --> X[X Timeline Provider]
  ADMIN --> LLM[Translation Provider]
...
保留 GitHub、Neon、Upstash、COS、EdgeOne 和 DNSPod，先让 VPS 承担应用进程。临时 quiz 不随 Realm/Admin 迁移；若活动尚未结束，应保持其独立的 Vercel/Redis 入口：

- Realm 运行 `pnpm install --frozen-lockfile`、`pnpm build` 和 `pnpm start`；Realm 的 `start` 会经过 `server.js`。
- Admin 运行其 `pnpm build` 与 `pnpm start`；Admin 当前使用 Next 默认生产 server。
- Docs 构建 `doc_build/`，使用 Nginx/Caddy 等静态服务器提供文件。
...
将 `/api/cron/x-timeline` 从 Vercel Cron 改为 systemd timer、宿主机 cron 或独立 worker。保持：
...
- 将 `@neondatabase/serverless`/`drizzle-orm/neon-http` 的直接连接点替换为标准 PostgreSQL driver/pool，并保持参数化查询、连接释放、事务和超时。
...
### 6. 暂时保留 GitHub Content

换 VPS 不要求立刻搬走内容仓库。第一阶段可以只把 `GITHUB_*` 变量迁到 VPS secret store，继续使用 GitHub 的历史、审计和 review。

若需要完全脱离 GitHub，按以下顺序实现 `ContentRepository`：

1. 定义 repo-relative path、branch/ref 和安全 path contract。
2. 定义 read file、list tree、write/delete with compare-and-swap、commit/audit 和 conflict response。
3. 保留 `blog/<slug>/<locale>.mdx`、`blog-index.json`、`tweets/index.json` 和月度 JSON 的外部格式，先做无损导入/导出。
4. 用离线导入工具验证所有 slug/locale、protected access、tweet count、translation 和 X origin。
5. 先让 Realm 只读新 provider，再让 Admin 写入；确认 revalidation 和回滚后再切换写入权。

不要为了绕开 GitHub API 而直接把内容散落到 Admin 数据库；那会同时改变历史、编辑冲突、备份和主站读取边界。
...
3. 降低 DNS TTL，准备 `arsvine.com`、`ctrl.arsvine.com`、`docs.arsvine.com`、`lab.arsvine.com` 的记录变更；`cdn.arsvine.com` 先保持独立。`quiz.arsvine.com` 属于临时活动记录，不纳入长期切换批次。
4. 先切 Admin 的受控入口，再切 Docs，最后切 Realm apex；每次记录 HTTP、auth、revalidate、content、asset 和 rollback 结果。
5. DNS 切换后保留旧 Vercel deployment 直到 cookie、ISR、CDN 和 revalidation 观察窗口结束。

Admin WebAuthn 的 `WEBAUTHN_RP_ID`/`WEBAUTHN_ORIGIN` 是安全契约。若继续使用 `ctrl.arsvine.com`，切换托管商不应改变 origin；若改变 origin，必须把它作为独立的凭据迁移项目处理。
...
| Admin Vercel project/plan 与 Cron dashboard             | `PARTIAL`        | CLI 已确认 `arsvine-admin`、Hobby、Node `24.x` 和 production deployment；仍需 Cron last-run/failure、preview protection 和自动部署规则 |
...
| `arsvine-docs` 是否仍承担未来公开架构文档               | `CLAIMED`/过时   | 用户确认文档公开范围和后续维护位置                                                                                                     |
...
| Realm/Admin production deployment 与当前仓库 HEAD 对齐 | `PASS`             | Vercel CLI：Realm `4b8f799`、Admin `a0cc4a0` 均为 `READY`                               |
...
| Content 可被本地 Git checkout 读取                     | `PASS`             | 本次浅克隆、结构统计、index/month count 校验                                            |
...
| 临时 quiz 活动站已清理                                 | `NOT_RUN`          | 活动尚未结束；不能删除共享 Upstash resource                                             |

## 适合当前阶段的最小动作

在真正决定迁移之前，持续收益最高的工作是把事实和边界固定下来：统一 Content index contract、归档 COS 原始输入、取得数据库/供应商后台的脱敏拓扑、为 GitHub/COS/Neon/Redis/X/translation 增加 provider contract tests。代码抽象应跟随已批准的迁移项目进入对应仓库，不为尚未决定的供应商预先建立通用框架。
