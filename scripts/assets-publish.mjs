...
async function cos(region, commandArgs) {
  ...
  if (dryRun) {
    console.log(`[assets:publish] dry-run ${printable}`);
    return;
  }
  return execFileAsync(coscli, [...clientArgs(region), ...commandArgs], {
    cwd: root,
    windowsHide: true,
  });
}
...
async function revalidate() {
  ...
  throw new Error(
    `Asset revalidation failed with HTTP ${response.status}${body.failed ? ` (failed: ${body.failed.join(', ')})` : ''}`,
  );
}
...
async function writePointer(version) {
  ...
  await cos(required('COS_PUBLIC_REGION'), [
    'cp',
    pointer,
  ...
  await cos(required('COS_PRIVATE_REGION'), [
    'cp',
    pointer,
...
async function main() {
  ...
  const privateListing = await cos(privateRegion, [
    'ls',
    `cos://${privateBucket}/${privateAssetKey}`,
  ]);
  if (!dryRun) {
