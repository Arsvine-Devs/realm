...
describe('responsive style migration', () => {
  it('keeps the Experience timeline mobile overrides with its feature', async () => {
    ...
    expect(source).toMatch(
      /@include breakpoints\.tablet-down[\s\S]*\.experienceTimeline::before[\s\S]*left: 8px/,
    );
    expect(source).toMatch(
      /@include breakpoints\.mobile[\s\S]*\.timelineItem[\s\S]*width: 100% !important/,
    );
  });
  ...
  it('keeps Contact mobile flow overrides with the profile feature', async () => {
    ...
    expect(source).toMatch(
      /@include breakpoints\.tablet-down[\s\S]*\.logItem[\s\S]*position: relative/,
    );
  ...
  it('keeps About mobile panel and decorative-image overrides with the profile feature', async () => {
    ...
    expect(source).toMatch(
      /@include breakpoints\.tablet-down[\s\S]*\.aboutNewImageWrapper[\s\S]*position: relative/,
    );
    expect(source).toMatch(
      /@include breakpoints\.mobile[\s\S]*\.contentSection\.aboutSection[\s\S]*padding: 20px 16px 80px/,
    );
  });
  ...
  it('keeps Friends full-width mobile section overrides with the profile feature', async () => {
    ...
    expect(source).toMatch(
      /\.friendLinkSection[\s\S]*@include breakpoints\.mobile[\s\S]*width: 100%/,
    );
    expect(source).toMatch(
      /\.friendLinkSection[\s\S]*@include breakpoints\.mobile[\s\S]*margin-left: 0/,
    );
    expect(source).toMatch(
      /\.friendLinkSection[\s\S]*@include breakpoints\.mobile[\s\S]*scroll-margin-top: var\(--mobile-section-scroll-offset\)/,
    );
    expect(source).toMatch(
      /\.friendLinkSection[\s\S]*@include breakpoints\.mobile[\s\S]*padding: 20px 16px 80px/,
    );
  });
  ...
  it('keeps shared detail primitives out of feature detail modules', async () => {
    ...
    expect(source).not.toMatch(
      /^\.(?:detailContainer|detailTitle|thumbnailGrid|inlineIconLink|iconRipple|copyableTextButton)\b/m,
    );
  });
});
